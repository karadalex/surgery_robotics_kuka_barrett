/**
 * Created by Alexios Karadimos on 16/3/20.
 *
 * This is a modified version of https://github.com/modulabs/arm-control/blob/master/arm_controllers/src/computed_torque_controller.cpp
 */
...
namespace kuka_barrett_control
{
	class ComputedTorqueControllerTest : public controller_interface::Controller<hardware_interface::EffortJointInterface>
	{
	public:
		bool init(hardware_interface::EffortJointInterface *hw, ros::NodeHandle &n)
		{
			...
			for (size_t i = 0; i < n_joints_; i++)
			{
				...
				if (n.getParam("/computed_torque_controller/gains/iiwa_joint_" + si + "/pid/p", Kp[i]))
				{
					Kp_(i) = Kp[i];
				}
				else
				{
					std::cout << "/computed_torque_controller/gains/iiwa_joint_" + si + "/pid/p" << std::endl;
					ROS_ERROR("Cannot find pid/p gain");
					return false;
				}

				if (n.getParam("/computed_torque_controller/gains/iiwa_joint_" + si + "/pid/i", Ki[i]))
				{
					Ki_(i) = Ki[i];
				}
				else
				...
				if (n.getParam("/computed_torque_controller/gains/iiwa_joint_" + si + "/pid/d", Kd[i]))
				{
					Kd_(i) = Kd[i];
				}
				else
...
// Export ROS plugin
PLUGINLIB_EXPORT_CLASS(kuka_barrett_control::ComputedTorqueControllerTest, controller_interface::ControllerBase)
