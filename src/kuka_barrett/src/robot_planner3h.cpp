//
// Created by karadalex on 30/12/21.
//
...
{
	ros::init(argc, argv, "robot_planner3h");
	...
	circleTrajCenter << 0.0, 0.0, -0.3; // Coordinates of desired circle w.r.t. to {F} reference frame
	HelixTrajectory* helixTrajectory = new HelixTrajectory(circleTrajCenter, 0.1, 3, 0.05);
	vector<geometry_msgs::Pose> helix_waypoints = helixTrajectory->getCartesianWaypoints(50, U_T_F, T_TCP_7);
	vector<geometry_msgs::Pose> transformed_waypoints = fulcrumEffectTransformation(helix_waypoints, 0.4, U_T_F, T_TCP_7);
	...
	// traj1.executeCartesianPath(path3, "Path to helix");
	traj1.executeCartesianPath(transformed_waypoints, "Helix Trajectory", false);

	traj1.visualizeCartesianPath(helix_waypoints, "Original taskspace helix trajectory", false);
