...
{
	ros::init(argc, argv, "robot_planner3g");
	...
	if (found_ik1 && found_ik2) {
		vector<SCurveSingleJointTrajectory*> joint_trajectories;
		for (int j = 0; j < 7; ++j) {
			double q1 = joint_values1[j];
			double q2 = joint_values2[j];
			double qddc, t1, t2, tau1, tau2;
			qddc = 2.0;
			t1 = 0.0;
			t2 = 1.0;
			tau1 = 0.1;
			tau2 = 0.1;
			SCurveSingleJointTrajectory* trajectory = new SCurveSingleJointTrajectory(q1, q2, t1, t2, tau1, tau2, qddc);
